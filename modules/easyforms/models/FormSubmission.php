...
use Carbon\Carbon;
use app\modules\easyforms\components\validators\DataValidator;
use app\modules\easyforms\components\analytics\enricher\IpLookupsEnrichment;
...
class FormSubmission extends ActiveRecord
{
    ...
    {
        return ['id', 'form_id', 'number', 'status', 'new', 'important',
    ...
    public function attributeLabels()
    {
        return [
            ...
            'new' => Yii::t('form', 'New'),
            'important' => Yii::t('form', 'Important'),
            'sender' => Yii::t('form', 'Sender'),
            'data' => Yii::t('form', 'Data'),
            'ip' => Yii::t('form', 'IP Address'),
    ...
    public function getSubmissionData()
    {
        ...
        if ($this->files && !empty($this->files)) {
            foreach ($this->files as $file) {
                $files[$file->field] = $file->getLink(true);
            }
        }
        ...
        return $data + [
                ...
                'ip_address' => $this->ip,
                'created_at' => is_null($this->created_at) ? Carbon::createFromTimestampUTC(time())->toDateTimeString() : Carbon::createFromTimestampUTC($this->created_at)->toDateTimeString(),
                'updated_at' => is_null($this->updated_at) ? Carbon::createFromTimestampUTC(time())->toDateTimeString() : Carbon::createFromTimestampUTC($this->updated_at)->toDateTimeString(),
            ] + $files + $sender + $referrer;
    }
