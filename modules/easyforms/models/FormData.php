...
use app\modules\easyforms\components\JsonToArrayBehavior;
use Yii;
...
class FormData extends ActiveRecord
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'builder' => Yii::t('form', 'Form Builder'),
            'fields' => Yii::t('form', 'Form Fields'),
            'html' => Yii::t('form', 'Form Html'),
     ...
     * @return string
     */
    private function getFieldLabel($field, $isGroup = false, $withAlias = false)
    {
        if ($isGroup) {
            if (isset($field['groupLabel'])) {
                $label = $field['groupLabel'];
            } else {
                $label = Yii::t('form', 'Group of') . " " . $field['name'];
            }
        } else {
            if (isset($field['label']) && !empty($field['label'])) {
                ...
                $label = Yii::t('form', 'Field with ID') . ": " . $field['id'];
                $withAlias = true;
            }
        }
        ...
        foreach ($fields as $field) {
            ...
            if (isset($field['type']) && $field['type'] === "radio") {
                ...
                $variable = [
                    ...
                    'label' => $this->getFieldLabel($firstRadio, true, true), // Get groupLabel
                    'fieldType' => 'radio',
                ];
                ...
                foreach ($radios as $radio) {
                    $option = [
                        "value" => $radio['value'],
                        "label" => $this->getFieldLabel($radio, false, true),
                    ];
            ...
            $variable = [
                ...
                'label' => $this->getFieldLabel($field, false, true),
    ...
    public function getRuleFields()
    {
        ...
        foreach ($fields as $field) {
            ...
            if (isset($field['type']) && ($field['type'] === "checkbox" || $field['type'] === "radio")) {
                ...
                $option = [
                    'name' => $field['name'],
                    'label' => $this->getFieldLabel($field, true, true), // Get groupLabel,
                ];
    ...
    public function getFieldsForValidation()
    {
        ...
        foreach ($allFields as $field) {
            if (!isset($field['label'])) {
                $field['label'] = isset($field['data-label']) && !empty($field['data-label']) ? $field['data-label'] : Yii::t('form', 'the input value');
            }
            if (isset($field['type']) && ($field['type'] == "checkbox" || $field['type'] == "radio") &&
                !isset($field['groupLabel'])) {
                $field['groupLabel'] = Yii::t('form', 'the input value');
            }
    ...
    {
        $fields = Json::decode($this->fields, true);
        return ArrayHelper::filter($fields, 'file', 'type');
    }
    ...
    public function getEmailLabels()
    {
        ...
        foreach ($emailFields as $field) {
            $labels[$field['name']] = $this->getFieldLabel($field);
        }
        return $labels;
    }
