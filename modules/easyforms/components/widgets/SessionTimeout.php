...
class SessionTimeout extends Widget
{
    ...
    public function init()
    {
        parent::init();
        if ($this->title === null) {
            $this->title = Yii::t('form', 'Session Timeout');
        }
        if ($this->message === null) {
            $this->message = Yii::t('form', 'Your online session will expire in');
        }
        if ($this->actionMessage === null) {
            $this->actionMessage = Yii::t('form', 'Please click "{keepAliveButton}" to keep working or click "{logoutButton}" to end your session now.');
        }
        if ($this->logoutButton === null) {
            $this->logoutButton = Yii::t('form', 'Sign out');
        }
        if ($this->keepAliveButton === null) {
            $this->keepAliveButton = Yii::t('form', 'Stay signed in');
        }
