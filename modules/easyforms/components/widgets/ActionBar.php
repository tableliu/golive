    ...
    protected function initDefaultElements()
    {
        if (!isset($this->elements['bulk-actions'])) {
            if ($this->bulkActionsPrompt === null) {
                $this->bulkActionsPrompt = Yii::t('form', 'Bulk Actions');
            }
            if (empty($this->bulkActionsItems)) {
                $this->bulkActionsItems = [
                    Yii::t('form', 'General') => [
                        'general-delete' => Yii::t('form', 'Delete'),
                    ],
                ];
            }
                ...
                $this->bulkActionsOptions = ArrayHelper::merge($this->bulkActionsOptions, [
                    'options' => [
                        'general-delete' => [
                            ...
                            'data-confirm' => Yii::t('form', 'Are you sure you want to delete these items?'),
                        ],
                    ],
                ]);
            }
        ...
        if (!isset($this->elements['create'])) {
            $this->elements['create'] = Html::a(
                '<span class="glyphicon glyphicon-plus-sign"></span> ' . Yii::t('form', 'Create New'),
                Url::toRoute('create'),
        ...
        return preg_replace_callback('/\\{([\w\-\/]+)\\}/', function ($matches) {
            ...
            if (isset($this->elements[$name])) {
                if ($name === 'bulk-actions' && $this->grid !== null) {
                    $id = $this->options['id'];
                    $message = Yii::t('form', 'Please select one or more items from the list.');
