...
namespace app\modules\easyforms\components\validators;
...
use app\modules\easyforms\components\rules\RuleEngine;
use app\modules\easyforms\components\rules\Finder;
...
class DataValidator
{
    ...
    public function uniqueFieldsValidation()
    {
        ...
        $message = Yii::t('form', '{attribute} "{value}" has already been taken.');
    ...
    public function requiredFieldsValidation()
    {
        ...
        // Messages
        $requiredMessage = Yii::t('form', 'the input value is required.');
                ...
                if ($key === "type" && $value === "email") {
                    ...
                    if (isset($field["name"], $this->data[$field["name"]]) &&
                        ...
                        if (isset($field["multiple"])) {
                            ...
                            if (count($emails) > 1) {
                                $config['message'] = Yii::t('form', '{attribute} has a invalid email format: Please use a comma to separate multiple email addresses.');
                            }
    ...
    public function addError($attribute, $label = '', $value = '', $error = '')
    {
        if (!empty($label) && !empty($value)) {
            $this->errors[$attribute][] = Yii::t('form', $error, [
                'attribute' => $label,
                'value' => $value,
            ]);
        } else {
            $this->errors[$attribute][] = str_replace(Yii::t('form', 'the input value'), $label, $error);
        }
    }
