...
class RecaptchaValidator extends Validator
{
    ...
    public function init()
    {
        ...
        $this->emptyMessage = Yii::t('form', 'The captcha is a required field.');
        $this->message = Yii::t('form', 'The captcha code you entered was incorrect.');

        if (empty($this->secret)) {
            $this->secret = Yii::$app->settings->get("app.reCaptchaSecret");
            if (empty($this->secret)) {
                throw new InvalidConfigException(Yii::t('form', 'ReCaptcha Secret Key is empty.'));
            }
        }
    }
    ...
    protected function validateValue($value)
    {
        ...
        if (!isset($response['success'])) {
            throw new \Exception(Yii::t('form', 'Invalid reCAPTCHA verification response.'));
        }
