...
namespace app\modules\easyforms\events\handlers;

use Yii;
use yii\base\Component;
use app\modules\easyforms\helpers\MailHelper;
use app\modules\easyforms\helpers\SubmissionHelper;
use app\modules\easyforms\events\SubmissionMailEvent;
...
class SubmissionEventHandler extends Component
{
    ...
    public static function onSubmissionAccepted($event)
    {
        ...
        $async = (boolean)Yii::$app->formsettings->get('async', 'app', $async);
        $async = $async && Yii::$app->formsettings->get('mailerTransport', 'app', 'php') === 'smtp';
        ...
        if (isset($formEmailModel->to) && isset($formEmailModel->from) && !empty($formEmailModel->to)) {
            ...
            $subject = isset($formEmailModel->subject) && !empty($formEmailModel->subject) ?
                ...
                $formModel->name . ' - ' . Yii::t('form', 'New Submission');
            ...
            $mailer->viewPath = Yii::$app->getModule("form")->emailViewPath;
            $message = $mailer->compose($notificationViews, $data)
                ->setFrom($fromEmail)
            ...
            if (!empty($formEmailModel->cc)) {
                $message->setCc(explode(',', $formEmailModel->cc));
            }

            // Add bcc
            if (!empty($formEmailModel->bcc)) {
                $message->setBcc(explode(',', $formEmailModel->bcc));
            }

            // Attach files
            if ($formEmailModel->attach && count($filePaths) > 0) {
                foreach ($filePaths as $filePath) {
                    $message->attach(Yii::getAlias('@app/web') . DIRECTORY_SEPARATOR . $filePath);
                }
            }
                ...
                if (!filter_var($to, FILTER_VALIDATE_EMAIL) === false) {
                    ...
                    $data = [
                        ...
                        !empty($formConfirmationModel->mail_message) ? $formConfirmationModel->mail_message :
                            Yii::t('form', 'Thanks for your message'),
                    ];

                    // Subject
                    $subject = isset($formConfirmationModel->mail_subject) && !empty($formConfirmationModel->mail_subject) ?
                        $formConfirmationModel->mail_subject : Yii::t('form', 'Thanks for your message');
                    ...
                    $replyTo = isset($formConfirmationModel->mail_from) && !empty($formConfirmationModel->mail_from) ?
                        $formConfirmationModel->mail_from : Yii::$app->formsettings->get("app.noreplyEmail");
