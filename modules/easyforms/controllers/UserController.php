...
class UserController extends Controller
{
                ...
                'rules' => [
                    ...
                    [
                        'actions' => ['account', 'profile', 'change-username', 'change-email',
                            'change-password', 'preferences', 'avatar-delete', 'resend-change', 'cancel'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                    [
    ...
    public function actionLoginEmail()
    {
        ...
        if ($loginEmailForm->load($post) && $loginEmailForm->sendEmail()) {
            ...
            $message = $user ? Yii::t('form', 'Log In link sent.') : Yii::t('form', 'Sign Up link sent.');
            $message .= ' ' . Yii::t('form', 'Please check your email.');
        ...
        if ($post = Yii::$app->request->post()) {
            if (isset($post['action']) && $post['action'] === 'session') {
                ...
                Yii::$app->getSession()->setFlash(
                    'success',
                    Yii::t('form', 'Your preferences have been successfully updated.')
                );
            }
        }
