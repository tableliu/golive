        ...
        if (confirm(options.i18n.areYouSureDeleteCommentItem)) {
            ...
            $.ajax({
                ...
                if (resp.success) {
                    ...
                    var oldComments = self.model.get('comments');
                    var newComments = _.reject(oldComments, function (comment) {
                        return comment.id == resp.commentID;
                    });
                    // Refresh comment list on Submission Model
                    self.model.set('comments', newComments);
                    // Reload this page
            ...
            if (this.modelExists) {
                files.each(function () {
                    ...
                    var FileUpload = Backbone.Model.extend({
                        url: options.uploadEndPoint + "&s_id=" + that.model.get('id'),
                        fileAttribute: field
                    });
                ...
                this.collection.create({
                    ...
                    success: function (model) {
                        files.each(function () {
                            ...
                            var field = $(this).attr('name');
                            var FileUpload = Backbone.Model.extend({
                                url: options.uploadEndPoint + "&s_id=" + model.get('id'),
                                fileAttribute: field
                            });
                            var fileUpload = new FileUpload();
                            var fileObject = $(this)[0].files[0];
            ...
            if (componentType === "selectlist") {
                ...
                that.$('select[name="' + key + '[]"]').each(function () {
                    ...
                    if (_.isString(selected)) {
                        that.$('select[name="' + key + '[]"]' + " option[value=\"" + selected + "\"]").prop("selected", true);
                    } else if (_.isArray(selected)) {
                        _.each(selected, function (val, k, list) {
                            console.log('select[name="' + key + '[]"]' + " option[value=\"" + val + "\"]");
                    ...
                    for (var countCanvas = 0; countCanvas < canvasElement.length; countCanvas++) {
                        (function (canvasEl) {
                            ...
                            var saveData = function () {
                                ...
                                that.$(fieldID).val(JSON.stringify(signature));
                            };

                            // Find the Hidden Element with canvas name
                            that.$(fieldID).val(data[key]);
                            signaturePad.onEnd = function () {
                                saveData();
                            };
                            // Color
                            var penColor = that.$('#' + canvasID).data('color');
                            if (penColor && penColor.length > 0 && penColor !== 'black') {
                                signaturePad.penColor = penColor;
                            }
                            // Clear
                            that.$(resetID).on("click", function () {
                                signaturePad.clear();
                                that.$(fieldID).val("");
                            });
                            // Undo
                            that.$(undoID).on("click", function () {
...
var Router = Backbone.Router.extend({
    ...
    edit: function (id) {
        ...
        this.main.html(this.views["navView"].render().el);
        this.views['formView'] = new App.FormView({
            id: id,
            subtitle: options.i18n.editSubmission,
            model: this.submissions.get(id)
        });
        this.main.append(this.views['formView'].render().el);
    },
    view: function (id) {
